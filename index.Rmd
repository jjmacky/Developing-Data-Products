...
website, but since users are shortening the same link with different apps,
...
Hours = seq(from=0, to=10*3600, by=3600)
By_Hand = seq(from=0, to=1000, by=100)
With_App = seq(from=0, to=10000, by=1000)
URLs = c(By_Hand,With_App)
data = cbind.data.frame("Method" = c(rep("By_Hand", times=11), rep("With_App", times = 11)),
                        "Number.of.URLs" = URLs, 
                        "Hours" = rep(Hours, times=2))
fig1 = Rickshaw$new()
fig1$layer(
  Number.of.URLs ~ Hours, 
  type = 'area', 
  data = data,
  groups = "Method",
...
fig1$save('assets/img/fig1.html', cdn = TRUE)
```
...
My app lets a user upload files in CSV or Excel format, specify the location
